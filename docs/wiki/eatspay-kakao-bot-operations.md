...
- 서버 이벤트 파일 `/opt/eatspay/uploads/kakao-tid-upload-events.json`은 `www-data:www-data`, `0664`를 운영 기준으로 한다. 저장 시 같은 디렉터리의 임시 파일을 원자적으로 교체하므로 기존 파일 소유권이 잘못돼도 디렉터리 쓰기 권한으로 복구된다.
- 동일 가맹점·동일 계좌의 승인 요청이 반복되면 가장 최근에 접수된 승인 기록만 활성 상태로 유지하고 이전 승인 기록은 자동으로 숨긴다. TID 업로드 매칭은 `hidden=false` 기록만 사용한다.
