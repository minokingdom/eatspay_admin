...
- TID 반영 결과의 `remainingValidationCount`가 `0`이고 배치 ID가 있을 때만 다음 다운로드/업로드 링크를 자동 생성한다.
...
- 완료된 배치 ID도 상태 파일에 보관하므로, 같은 배치의 0건 결과가 다시 조회되거나 서비스가 재시작되어도 자동 링크를 반복 발송하지 않는다.
