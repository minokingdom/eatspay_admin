...
test('extracts unique 8 to 20 digit candidates', () => {
  assert.deepEqual(extractAccountCandidates('고객 12345678 / 계좌 562 169754 32139 / 12345678'), ['12345678', '56216975432139']);
});
...
test('returns not_found when OCR text has no account candidate', () => {
  assert.deepEqual(compareAccountText('계좌번호를 읽지 못함', '56216975432139'), { status: 'not_found', recognizedAccountNo: '', candidates: [] });
});
...
test('runs tesseract with timeout and a numeric whitelist', async () => {
  const calls = [];
  const result = await recognizeAccountProof('/tmp/proof.jpg', '12345678', {
    runner: async (...args) => { calls.push(args); return '입금 계좌 1234-5678'; }
  });
