...
function normalizeProviderName(value = '') {
  ...
  if (['routeup', 'route', '위루트'].includes(compact)) return '위루트';
  return raw;
}
...
function normalizePgContract(contract = {}) {
  ...
  const credentialType = clean(contract.credentialType || contract.type || contract.usage || (providerName === '위루트' ? 'routeup' : 'recurring')) || 'recurring';
  ...
  return {
    ...
    paymentKey: clean(contract.paymentKey || contract.payKey || contract.key || (providerName === '위루트' ? '' : contract.apiKey)),
...
function hasRouteupExternalKeys(contract = {}) {
  ...
  return normalizeProviderName(normalized.providerName) === '위루트' && Boolean(
...
function hasCompleteRouteupExternalKeys(contract = {}) {
  const normalized = normalizePgContract(contract);
  const metadata = normalized.metadata && typeof normalized.metadata === 'object' ? normalized.metadata : {};
  if (normalizeProviderName(normalized.providerName) !== '위루트') return false;
  const apiKey = clean(
...
function hasRouteupExternalIntegrationKeys(account = {}) {
  ...
  return providerName === '위루트' && hasCompleteRouteupExternalKeys({ ...account, providerName: '위루트' });
}
...
function hasBillablePgContract(account = {}) {
  ...
  if (contracts.some(contract => {
    if (normalizeProviderName(contract.providerName) === '위루트') {
