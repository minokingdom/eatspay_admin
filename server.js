...
app.post('/api/admin/accounts/proof-ocr', authenticateAdmin, asyncHandler(async (req, res) => {
  if (accountProofOcrBusy) {
    return sendError(res, 429, 'OCR_BUSY', '다른 증빙을 인식하고 있습니다. 잠시 후 다시 시도해주세요.');
  }
  const accountNo = normalizeOcrAccountNo(req.body?.accountNo);
  if (accountNo.length < 8 || accountNo.length > 20) {
    return sendError(res, 400, 'INVALID_ACCOUNT_NO', '계좌번호는 숫자 8~20자리여야 합니다.');
  }
  ...
  try {
    const result = await recognizeAccountProof(imagePath, accountNo);
    return res.status(200).json({
      success: true,
      data: {
        status: result.status,
        registeredAccountNo: accountNo,
        recognizedAccountNo: result.recognizedAccountNo || '',
