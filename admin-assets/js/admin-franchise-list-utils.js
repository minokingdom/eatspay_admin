(function () {
  ...
  function pgSecretText(contract = {}, field = 'payment') {
  ...
  function routeupExternalKeyValue(contract = {}, field = 'api') {
  ...
  function routeupExternalKeyStatus(contract = {}, field = 'api') {
    return routeupExternalKeyValue(contract, field) ? '<span class="tg">등록완료</span>' : '<span class="franchise-muted-strong">미등록</span>';
  }

  function hasCompleteRouteupExternalKeys(contract = {}) {
    return normalizeProviderName(contract.providerName) === '위루트' &&
      Boolean(routeupExternalKeyValue(contract, 'api')) &&
      Boolean(routeupExternalKeyValue(contract, 'encryption')) &&
      Boolean(routeupExternalKeyValue(contract, 'iv'));
  }

  function hasRouteupExternalIntegrationKeys(account = {}, franchise = {}) {
    ...
    const provider = normalizeProviderName(account.providerName || account.pgProviderName || franchise.pgProviderName || '');
    return provider === '위루트' && hasCompleteRouteupExternalKeys({ ...account, providerName: '위루트' });
  }

  function accountPgContractMetaHtml(account = {}, franchise = {}, context = {}) {
    if ((context.role || '') !== 'hq' && context.isHq !== true) return '';
    const contracts = Array.isArray(account.pgContracts) ? account.pgContracts : [];
    ...
    if (routeup || selectedProvider === '위루트') {
      ...
      const optionalPaymentKey = contract.paymentKey || contract.paymentKeyMasked || contract.key || contract.hasPaymentKey
        ? `<div class="admin-mono">결제 KEY: <span class="tg">등록완료</span></div>`
        : '';
      return `
    ...
    return `
            <div class="admin-mono">PG: ${esc(normalizeProviderName(franchise.pgProviderName) || 'GH Payments')}</div>
  ...
  function cardProviderId(card = {}) {
    return String(card.pgProviderId || card.pg_provider_id || card.providerId || '').trim();
  }

  function franchiseProviderId(franchise = {}) {
    return String(franchise.pgProviderId || franchise.pg_provider_id || franchise.providerId || '').trim();
  }

  function isCurrentVisibleCard(franchise = {}, card = {}) {
    return Boolean(card && card.hidden !== true && card.active !== false);
  }

  function visibleCardItems(franchise = {}, items = null) {
    const source = Array.isArray(items)
      ? items
      : (Array.isArray(franchise.cardList) ? franchise.cardList.filter(Boolean) : []);
    return source.filter(card => isCurrentVisibleCard(franchise, card));
  }
  ...
  function sortedDisplayLines(franchise = {}, context = {}) {
    ...
    return Array.from({ length: count }, (_, index) => ({
      index,
      card: cards[index]?.card || null,
      accountEntry: accounts[index] || accounts[accounts.length - 1] || null,
      matchedPaymentDate: cards[index]?.matchedPaymentDate || ''
    }));
  }
  ...
  function visibleAccountStatus(franchise = {}, context = {}) {
    ...
    if (context.role && context.role !== 'hq' && context.isHq !== true) {
      ...
      if (accounts.some(account => account.accountStatus === '승인완료')) return '승인완료';
      return accounts[0]?.accountStatus || '미등록';
    }
    if (accounts.some(account => account.accountStatus === '반려')) return '반려';
    if (accounts.some(account => account.accountStatus === '승인대기' && !hasAccountApprovalCredentials(account, franchise))) return '승인대기';
    if (accounts.some(account => account.accountStatus === '승인완료' && hasAccountApprovalCredentials(account, franchise))) return '승인완료';
    if (accounts.some(account => account.accountStatus === '승인대기' && hasAccountApprovalCredentials(account, franchise))) return '승인완료';
    if (accounts.some(account => account.accountStatus === '승인완료')) return '승인대기';
    return accounts[0]?.accountStatus || '미등록';
  }

  function buildFranchiseExportRows(franchises = [], context = {}) {
    const source = Array.isArray(franchises) ? franchises : [];
    const visibleAccountEntries = typeof context.visibleAccountEntries === 'function'
      ? context.visibleAccountEntries
      : franchise => (Array.isArray(franchise.deliveryAgencies) ? franchise.deliveryAgencies : [])
        .filter(account => account && account.hidden !== true)
        .map((account, idx) => ({ account, idx }));
    const cardItems = typeof context.cardItems === 'function'
      ? context.cardItems
      : franchise => visibleCardItems(franchise);
    const accountStatus = typeof context.accountStatus === 'function'
      ? context.accountStatus
      : franchise => visibleAccountStatus(franchise, context);
    const resolveAccountDisplayStatus = typeof context.accountDisplayStatus === 'function'
      ? context.accountDisplayStatus
      : (account, franchise) => accountDisplayStatus(account, franchise, context);
    const bankDisplay = typeof context.bankDisplay === 'function'
      ? context.bankDisplay
      : account => account.bankName || account.bank || '';
    const displayMemberId = typeof context.displayMemberId === 'function'
      ? context.displayMemberId
      : value => String(value || '').split('@')[0];
    const dateOnly = typeof context.dateOnly === 'function'
      ? context.dateOnly
      : value => String(value || '').slice(0, 10);
    const latestPaymentDate = typeof context.latestPaymentDate === 'function'
      ? context.latestPaymentDate
      : franchise => sortedDisplayLines(franchise, context)
        .map(displayLineDate)
        .filter(Boolean)
        .sort()
        .pop() || '';
    const cardPaymentDate = typeof context.cardPaymentDate === 'function'
      ? context.cardPaymentDate
      : (franchise, card) => paymentDateForCard(franchise, card, context);

    const summaryRows = source.map(franchise => {
      const accounts = visibleAccountEntries(franchise);
      const cards = cardItems(franchise);
      return {
        franchiseId: String(franchise.id || ''),
        franchiseName: franchise.name || '',
        loginId: franchise.loginId || franchise.customerId || displayMemberId(franchise.email) || '',
        agency: franchise.agency || (typeof context.agencyName === 'function' ? context.agencyName(franchise) : '') || '',
        owner: franchise.owner || '',
        phone: franchise.phone || '',
        joinDate: franchise.joinDate || dateOnly(franchise.createdAt) || '',
        accountStatus: accountStatus(franchise),
        accountCount: accounts.length,
        cardCount: cards.length,
        recentPayment: latestPaymentDate(franchise),
        memberStatus: franchise.status || franchise.role || ''
      };
    });

    const accountRows = source.flatMap(franchise => visibleAccountEntries(franchise).map(entry => {
      const account = entry?.account || entry || {};
      return {
        franchiseId: String(franchise.id || ''),
        franchiseName: franchise.name || '',
        agency: franchise.agency || (typeof context.agencyName === 'function' ? context.agencyName(franchise) : '') || '',
        deliveryAgency: account.agency || account.agencyName || franchise.deliveryAgency || franchise.deliveryCompany || '',
        accountStatus: resolveAccountDisplayStatus(account, franchise),
        bankName: bankDisplay(account),
        accountNo: account.accountNo || account.virtualAccountNo || '',
        accountHolder: account.accountHolder || account.depositor || '',
        recurringTid: account.recurringTid || account.txid || '',
        recurringKey: account.recurringKeyMasked || '',
        manualTid: account.manualTid || '',
        manualKey: account.manualKeyMasked || ''
      };
    }));

    const cardRows = source.flatMap(franchise => cardItems(franchise).map(card => ({
      franchiseId: String(franchise.id || ''),
      franchiseName: franchise.name || '',
      agency: franchise.agency || (typeof context.agencyName === 'function' ? context.agencyName(franchise) : '') || '',
      alias: card.alias || card.cardAlias || '',
      cardCompany: card.cardCompany || card.cardName || '',
      cardLast4: String(card.cardLast4 || card.maskedNumber || card.cardNumber || '').replace(/[^0-9]/g, '').slice(-4),
      cardStatus: card.hidden === true || card.active === false ? '숨김' : (card.status || card.cardStatus || '사용중'),
      recentPayment: cardPaymentDate(franchise, card)
    })));

    return { summaryRows, accountRows, cardRows };
  }

  function hasAccountTidKey(account = {}, providerName = '') {
    const selectedProvider = normalizeProviderName(providerName);
    const contracts = Array.isArray(account.pgContracts) ? account.pgContracts : [];
    if (contracts.some(contract => {
      const provider = normalizeProviderName(contract.providerName);
      if (selectedProvider && provider !== selectedProvider) return false;
      if (provider === '위루트') return Boolean(contract.tid && (contract.hasPaymentKey || contract.paymentKey || contract.paymentKeyMasked));
    ...
    if (selectedProvider && selectedProvider !== 'GH Payments') return false;
  ...
  function hasAccountApprovalCredentials(account = {}, franchise = {}) {
    const provider = normalizeProviderName(account.pgProviderName || franchise.pgProviderName || '');
    if (provider === '위루트') return hasRouteupExternalIntegrationKeys(account, franchise) && hasAccountTidKey(account, '위루트');
    if (provider === 'GH Payments') return hasAccountTidKey(account, 'GH Payments');
    return hasAccountTidKey(account);
  }
  ...
  function accountDisplayStatus(account, franchise = {}, context = {}) {
    ...
    if (context.role && context.role !== 'hq' && context.isHq !== true) return raw;
    if (raw === '승인대기' && hasAccountApprovalCredentials(account, franchise)) return '승인완료';
    if (raw === '승인완료' && !hasAccountApprovalCredentials(account, franchise)) return '승인대기';
    return raw;
  }
  ...
  function listAccountStatusStack(franchise = {}, lineCount = 1, lines = null, context = {}) {
    ...
    return lineStack(dedupedAccountLines(displayLines, line => {
      ...
      if ((context.role === 'hq' || context.isHq === true) && accountStatus === '승인대기' && line.accountEntry) {
  ...
  function noteInlineHtml(franchise = {}, context = {}) {
    const isHq = context.role === 'hq' || context.isHq === true;
    if (!isHq) return '';
    return `<div class="franchise-note-inline"><textarea class="fi franchise-note-input" data-fr-note-input="1" data-fr-id="${esc(franchise.id || '')}" maxlength="1000" rows="3" placeholder="가맹점 관련 메모를 입력하세요">${esc(franchise.note || '')}</textarea><button type="button" class="btn bg2 xs franchise-note-save" data-admin-action="fr-note-save" data-fr-id="${esc(franchise.id || '')}">저장</button></div>`;
  }
  ...
  function basicDetailHtml(franchise = {}, context = {}) {
    ...
    const noteRow = (context.role === 'hq' || context.isHq === true)
      ? infoRow('비고', noteInlineHtml(franchise, context), true)
      : '';
  ...
  function cardDetailHtml(franchise = {}, context = {}) {
    const rawCardItems = typeof context.cardItems === 'function'
      ? context.cardItems(franchise)
      : (Array.isArray(franchise.cardList) ? franchise.cardList.filter(Boolean) : []);
    const cardItems = visibleCardItems(franchise, rawCardItems);
      ...
      ? accounts.map((account, index) => {
        ...
        return `<div class="franchise-account-item">
          ...
          <div class="franchise-account-meta">
            ...
            ${accountPgContractMetaHtml(account, franchise, context)}
            <div>증빙: ${proof}</div>
          </div>
        </div>`;
      }).join('')
  ...
  window.EatsAdminFranchiseListUtils = Object.assign(root, {
    ...
    paymentDateForCard,
    visibleCardItems,
    isCurrentVisibleCard,
    sortedDisplayLines,
    buildFranchiseExportRows,
