...
모든 앱/관리자 화면 수정 전에는 [`docs/ui-consistency-directive.md`](docs/ui-consistency-directive.md)와 [`design.md`](design.md)를 우선 확인합니다.

* 앱과 관리자 기본 폰트는 모두 `Pretendard`입니다.
* 메인 초록색은 `#03c75a`를 우선 사용합니다.
* 기존에 완성된 화면을 먼저 찾아 기준으로 삼고, 새 스타일을 즉흥적으로 만들지 않습니다.
* HTML 구조, 버튼, 필터, 테이블, 모달, 배지 패턴은 `design.md`의 예시 구조를 따릅니다.
* `style=""` 인라인 CSS는 새로 작성하지 않습니다. 불가피하게 인라인 스타일이 필요하다고 판단되면 수정 전에 사용자에게 이유와 적용 위치를 설명하고 승인을 받은 뒤 진행합니다.
* `onclick=""` 직접 바인딩을 새로 늘리지 않습니다.
...
* 계좌 증빙/포스사진 같은 첨부 파일은 내부 저장 키와 사용자 표시명을 분리합니다. 내부 저장 키는 `timestamp-uuid.ext`를 유지하고, 관리자/사용자 표시명은 서버가 생성한 이름을 사용합니다.
* 가맹점 계좌 증빙 표시명은 `가맹점명_포스사진01.ext`부터 시작하고, 사업자등록증 표시명은 `가맹점명_사업자등록증01.ext`부터 시작합니다. 같은 가맹점명/첨부종류 기준 기존 첨부 표시명이 있으면 `02`, `03`처럼 두 자리 순번을 증가시킵니다. 클라이언트의 원본 파일명(`file.name`)을 최종 표시명으로 저장하거나 보여주지 않습니다.
...
이 프로젝트는 `graphify-out/graph.json`에 로컬 AST 기반 지식 그래프가 있습니다.

코드베이스 질문, 영향 범위 확인, 함수/파일 관계 추적, 구조 검색이 필요하면 `rg`나 전체 파일 읽기 전에 Graphify로 먼저 범위를 좁힙니다. Graphify 결과는 출발점이며, 최종 판단 전에는 반드시 실제 파일을 열어 확인합니다.

Windows 실행 명령은 `graphify.cmd`를 우선 사용합니다. 현재 환경에서 명령이 잡히지 않으면 전체 경로 `C:\Users\미노스\AppData\Roaming\npm\graphify.cmd`를 사용합니다.

규칙:
* 사용자가 `/graphify`를 입력하면 다른 작업보다 먼저 graphify 스킬/명령을 사용합니다.
* 코드베이스 질문은 `graphify-out/graph.json`이 있으면 먼저 `graphify.cmd query "<질문>"`을 실행합니다.
* 두 함수, 파일, 개념의 연결 관계는 `graphify.cmd path "<A>" "<B>"`로 확인합니다.
* 특정 함수나 개념의 주변 맥락은 `graphify.cmd explain "<concept>"`로 확인합니다.
* 변경 영향 범위는 `graphify.cmd affected "<node>"`로 먼저 확인한 뒤 실제 호출부를 파일에서 검증합니다.
* `graphify-out/wiki/index.md`가 있으면 넓은 구조 탐색에 먼저 사용합니다.
* `graphify-out/GRAPH_REPORT.md`는 큰 아키텍처 리뷰나 query/path/explain 결과가 부족할 때만 읽습니다.
* hook 또는 업데이트 뒤 `graphify-out/` 파일이 dirty인 것은 정상입니다. 그래프가 오래됐거나 사용자가 쓰지 말라고 한 경우가 아니면 이것만으로 Graphify를 건너뛰지 않습니다.
* 코드 수정 후에는 `graphify.cmd update . --no-cluster --force`를 실행해 로컬 AST 그래프를 갱신합니다. 이 명령은 외부 LLM/API 없이 동작합니다.
